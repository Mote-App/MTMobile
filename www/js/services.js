...
.factory('Profile', ['$resource', '$rootScope',
 ...
 .factory('fbSync', ['$resource', '$rootScope',
  function($resource, $rootScope){
    return $resource($rootScope.clhost + $rootScope.clport + '/fb/login', {}, {
                     sync: {method:'POST', params:{userId: '@userId', accessToken: '@accessToken'}}
    });
  }])
    ...
    var service = {
        ...
        login: function () {
            ...
            igPopup.addEventListener('loadstart', function(event) { 
                if((event.url).startsWith("http://localhost:8080/callback")) {
